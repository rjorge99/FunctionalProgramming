...
const tableHeader = thead([
    tr([cell(th, 'pa2 tl', 'Meal'), cell(th, 'pa2 tr', 'Calories'), cell(th, '', '')])
]);
...
function tableView(dispatch, meals) {
    if (meals.length === 0) return div({ className: 'mv2 i black-50' }, 'No meals yet...');
...
function formView(dispatch, model) {
    ...
    return button(
        { className: 'f3 pv2 ph3 bg-blue white bn', onclick: () => dispatch(showFormMsg(true)) },
        'Add Meal'
    );
}
